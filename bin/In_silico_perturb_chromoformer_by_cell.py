...
with torch.no_grad():
    for assay_i, gene_type_i,location_i in runs:
        ...
        for ind,fold in enumerate([x+1 for x in range(k_fold)]):
            #in case prev failed - don't rerun if already saved
            file = f"{PRED_PATH}/{cell_i}_{str(ind_test)}_fold{str(ind)}_chromoformer_in_silico_mut.csv"
            if not os.path.isfile(file):
                ...
                print(file,"exists, not reprocessing.")
        ind_test+=1