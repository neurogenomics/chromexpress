...
#loop through each decile and test (we have values in quantiles so just take two at a time)
for quant_i in quants:
    print("Quantile: ",quant_i," ------")
    ...
    for cell_i in cell_tested:
        ...
        #get quant i
        exp_gtex_cell_i_top = exp_gtex_cell_i[(exp_gtex_cell_i['quantiles']==quant_i)][['gene','cell_name',
                                                                                        'cell_anatomy_grp',
    ...
    bst_qtl_res = pd.DataFrame({"quantile":[quant_i] * len(p_vals),
                                ...
                                "p_value":p_vals,
                                "FDR_p":p_adjust})
    all_quant_res.append(bst_qtl_res)
all_quant_res = pd.concat(all_quant_res)    
all_quant_res.to_csv("./qtl_ovrlp/all_quant_bootstrap_qtl_res.csv",index=False)